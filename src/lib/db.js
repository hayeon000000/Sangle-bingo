...
export async function createRoom({ password, duration, winnerCount, topics }) {
  ...
  const { data, error } = await supabase
    .from('rooms')
    .insert({
      id, password: password || null, duration, winner_count: winnerCount,
      topics, status: 'waiting', master_key: masterKey, created_at: now,
    })
    .select().single()
...
export async function startRoom(roomId) {
  const now = Date.now()
  const { error } = await supabase.from('rooms').update({ status: 'playing', started_at: now }).eq('id', roomId)
  if (error) throw error
  return now
}

export async function endRoom(roomId) {
  const { error } = await supabase.from('rooms').update({ status: 'finished', ended_at: Date.now() }).eq('id', roomId)
  if (error) throw error
}
...
export async function joinRoom({ roomId, nickname, emoji, board }) {
  ...
  const { data, error } = await supabase
    .from('participants')
    .insert({
      id, room_id: roomId, nickname, emoji, board: boardForDb,
      completed_count: 0, bingo_lines: 0, joined_at: Date.now(),
    })
    .select().single()
  if (error) throw error
  return { ...dbParticipantToState(data), board } 
}

export async function fetchParticipants(roomId) {
  const { data, error } = await supabase.from('participants').select('*').eq('room_id', roomId).order('completed_count', { ascending: false })
  if (error) throw error
...
export async function updateParticipantBoard(participantId, board, completedCount, bingoLines) {
  ...
  const { error } = await supabase.from('participants').update({
    board: boardForDb, completed_count: completedCount, bingo_lines: bingoLines,
  }).eq('id', participantId)
  if (error) throw error
}
...
export function dbRoomToState(row) {
  return {
    id: row.id, password: row.password, duration: row.duration, winnerCount: row.winner_count,
    topics: row.topics, status: row.status, masterKey: row.master_key,
...
export function dbParticipantToState(row) {
  return {
    id: row.id, roomId: row.room_id, nickname: row.nickname, emoji: row.emoji,
    board: row.board ?? [], completedCount: row.completed_count,
    bingoLines: row.bingo_lines, joinedAt: row.joined_at,
  }
}
...
export function savePhotoLocally(participantId, cellIndex, dataUrl) {
  try { localStorage.setItem(PHOTO_KEY(participantId, cellIndex), dataUrl) } catch (e) {}
}

export function loadPhotoLocally(participantId, cellIndex) {
  try { return localStorage.getItem(PHOTO_KEY(participantId, cellIndex)) } catch { return null }
}
...
function stripPhotosFromBoard(board) {
  return board.map(cell => ({ ...cell, photo: cell.photo ? true : null }))
}

const SESSION_KEY = 'pb_session'
export function saveSession(data) { try { localStorage.setItem(SESSION_KEY, JSON.stringify(data)) } catch {} }
