...
import { useGame, shuffleArray, buildBoard } from '../contexts/GameContext'
...
export default function JoinRoomPage() {
  ...
  const handleJoin = () => {
    ...
    // Verify room
    if (!state.room || state.room.id !== roomId.trim().toUpperCase()) {
      return setError('존재하지 않는 방입니다.')
    }
    if (state.room.password && state.room.password !== password) {
      return setError('비밀번호가 틀렸습니다.')
    }
    if (state.room.status === 'finished') {
      return setError('이미 종료된 게임입니다.')
    }
    ...
    const shuffledTopics = buildBoard(shuffleArray(state.room.topics))
    joinRoom(participant, shuffledTopics)
  }
      ...
      <div className="max-w-sm mx-auto w-full px-4 pt-8 flex flex-col gap-6">
        ...
        <div className="card p-5 flex flex-col gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="input-field font-mono tracking-widest text-center text-xl uppercase"
        ...
        <div className="card p-5 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-10 gap-1.5">
              {EMOJIS.map(e => (
                <button
                  key={e}
                  onClick={() => setEmoji(e)}
                  className={`text-xl aspect-square rounded-lg flex items-center justify-center transition-all active:scale-90 ${
                    emoji === e
                      ? 'bg-acid/20 ring-2 ring-acid scale-110'
          ...
          <div className="flex items-center gap-3 bg-ink-700 rounded-xl p-3">
            <div className="w-12 h-12 rounded-xl bg-ink-600 flex items-center justify-center text-2xl">
              {emoji}
            </div>
            <div>
