...
export function GameProvider({ children }) {
  ...
  // Realtime 구독 (방장, 참여자 모두 룸 상태와 참여자 현황을 실시간으로 받음)
  useEffect(() => {
  ...
  const updateCell = useCallback(async (cellIndex, photo, timestamp) => {
    ...
    // 로컬 스토리지에 원본 사진 저장 및 DB 동기화 (비동기)
  ...
  const reset = useCallback(() => {
    db.clearSession()
    setState({ view: 'home', room: null, isMaster: false, currentParticipant: null, myBoard: null, isLoading: false })
  }, [])
